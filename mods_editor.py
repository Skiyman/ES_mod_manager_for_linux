...
class ModsEditor(object):
    ...
    def load_mods_db(self):
        ...
        for mod_id in self.mods_id:
            if mod_id not in self.mods_db or self.mods_db[mod_id]["mod_name"] == self.connection_error_name:
                if self.check_connection():
                    ...
                    name = self.connection_error_name
    ...
    def mod_replace(self, mod_id):
        self.mods_db = self.load_mods_db()
...
class MainWindow(Ui_Es_mod_namager_linux, QMainWindow):
    ...
    def enable_all(self, item):

        editor = ModsEditor()
        mods_db = editor.load_mods_db()
        for mod_id in mods_db:
            if mods_db[mod_id]["Work_status"] == "Disabled":
                editor.mod_replace(mod_id)

        self.fill_mods_lists()

    def disable_all(self, item):

        editor = ModsEditor()
        mods_db = editor.load_mods_db()
        for mod_id in mods_db:
            if mods_db[mod_id]["Work_status"] == "Enabled":
                editor.mod_replace(mod_id)
    ...
    def run_es(self):
        subprocess.call('steam steam://rungameid/331470 %U', shell=True)
